...
// Define formatSeconds globally for components like Timer
window.formatSeconds = (totalSecs) =>
  new Date(totalSecs * 1000).toISOString().slice(11, 19);

function App() {
  // Session / Auth state (restoring token from sessionStorage if exists)
  const [token, setToken] = useState(() => sessionStorage.getItem('study_token'));
  ...
  const stopAndSaveTimer = async (taskName) => {
    ...
    try {
      ...
      alert(`Successfully logged ${window.formatSeconds(timerSeconds)} of study time!`);
    } catch (err) {
      console.error(err);
    }
